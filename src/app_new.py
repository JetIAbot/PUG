...
def create_app(config=None):
    ...
    def extraer_datos():
        try:
            ...
            # Validar datos
            errors = validator.validate_student_form({
                'matricola': matricola,
                'password': password
            })
            ...
            if errors:
                security_logger.log_security_event(
                    'validation_failed',
                    {'errors': errors, 'matricola': matricola},
                    matricola,
                    request.remote_addr
                )
                return jsonify({
                    'titulo': 'Error de Validación',
                    'salida': 'Datos no válidos: ' + ', '.join(errors)
                }), 400
...
def init_celery_tasks(celery_app):
    ...
    def extraer_datos_task_impl(matricola, password):
        try:
            from src.main import extraer_datos_estudiante
